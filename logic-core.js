...
// Kapselt: Eingangsverwaltung, Negierung, Speicherzeit, Reset, Ausgabe auf Änderung
...
module.exports = function makeLogoLogicNode(RED, { typeName, displayName, computeResult }) {
    return function(config) {
        ...
        function evaluate() {
            // Negierung anwenden (fehlende negation flags = false)
        ...
        node.on("input", (msg, send, done) => {
            try {
                ...
                // Eingang identifizieren (1..inputsCount)
                let idx = undefined;
                if (msg.topic !== undefined) {
                    const n = parseInt(msg.topic, 10);
                    if (!isNaN(n)) idx = n - 1; // 0-basierend intern
                }
                if (idx === undefined || idx < 0 || idx >= inputsCount) {
                    // Ungültiges oder fehlendes Topic → ignorieren
                    if (done) done();
                    return;
                }

                // Wert holen und speichern
                ...
                if (val) {
                    ...
                    // explizites false → sofort zurücksetzen, unabhängig von memory
                    states[idx] = false;
                    lastTrueAt[idx] = 0;
                }

                // Memory auslaufen lassen
                expireMemory(now);

                // Ergebnis berechnen & senden
